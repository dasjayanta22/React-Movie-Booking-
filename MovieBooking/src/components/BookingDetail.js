...
import CalenderClass from './Calender/CalenderMain';
...
import BookingStyles from './BookingDetailStyle';
...
function getStepContent(stepIndex,date,hall,seat) {
  ...
  switch (stepIndex) {
    case 0:
      return (<CalenderClass className={BookingStyles.calender}/>);
    case 1:
      return <ShowTiming/>;
    case 2:
      return <SeatPickerMain />;
    default:
  return (<div style={{color:'black', display: 'flex', justifyContent: 'center' }}>
        <ul>
          <li>{date}</li>
          <li>{hall?hall : "RDx Cinemas : 10:00AM"}</li>
          <li>{seat}</li>
...
const BookingDetail =({selectedBookingDate, selectedTimeAndHall, selectedSeat}) =>{
  ...
  const handleNext = () => {
    ...
    console.log('selectedTimeAndHall', selectedTimeAndHall)
    ...
    <div className={classes.root}>
      ...
      <div>
        {activeStep === steps.length ? (
             ...
             <div style={{margin:'20px', textAlign:'center', color: 'black'}}>
            <h3>Congratulations Your Booking confirmed</h3>
            <p>{selectedBookingDate}</p>
            <p>{selectedTimeAndHall? selectedTimeAndHall : "RDx Cinemas : 10:00AM"}</p>
            <p>{selectedSeat}</p>
      ...
      >
              ...
              <Button variant="contained" disabled={proceed} color="primary" onClick={handleNext}>
